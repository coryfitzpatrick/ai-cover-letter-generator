    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a precise job posting analyzer. Extract requirements exactly as requested.",
                },
        ...
        if requirements_match:
            ...
            req_lines = re.findall(
                r"\d+\.\s*(\w+):\s*(.+?)\s*\(priority:\s*(\d+)\)", req_text, re.IGNORECASE
            )
        ...
        print(
            f"  Technologies: {', '.join(key_technologies) if key_technologies else 'None specific'}"
        )
